...
DEBUG VERSION — с полным логированием всех операций
"""
import logging
import requests
import json
...
class Config:
    ...
    LOG_FILE = os.getenv("LOG_FILE", "logs/telegram_bot_debug.log")
...
def create_bot():
    logger.info("=" * 60)
    logger.info("🤖 Запуск Telegram-бота IDO SKILLS News (DEBUG)")
    logger.info("=" * 60)
...
def get_user_by_telegram_id(telegram_id: int):
    ...
    try:
        url = f"{config.BACKEND_URL}/users/{telegram_id}"
        ...
        logger.error(f"❌ Ошибка подключения к бэкенду: {e}")
        logger.error(f"💡 Бэкенд доступен? {config.BACKEND_URL}")
        return None
    except Exception as e:
...
def create_user_in_backend(telegram_id: int, username: str):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            logger.error(f"❌ Ошибка 500: {response.text}")
            # Возможно пользователь уже существует
            logger.info("🔍 Пробуем найти существующего пользователя...")
        ...
        logger.error(f"❌ Ошибка подключения к бэкенду: {e}")
        logger.error(f"💡 Проверьте, запущен ли бэкенд: {config.BACKEND_URL}")
        return None
    except Exception as e:
...
def cmd_start(message):
    ...
    logger.info(f"👤 First name: {message.from_user.first_name}")
    logger.info(f"👤 Last name: {message.from_user.last_name}")
    logger.info("=" * 60)
    ...
    if not user:
        ...
        if user:
            logger.info(f"✅ Пользователь успешно создан: ID={user.get('id')}")
            bot.reply_to(
                message,
            ...
            bot.reply_to(
                ...
                f"• База данных существует\n"
                f"• Нет конфликта email",
                reply_markup=main_keyboard()
            )
    else:
