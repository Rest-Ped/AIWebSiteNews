"""Telegram bot for IDO SKILLS News with linked-user auth and AI chat."""
...
load_dotenv(BASE_DIR / ".env.bot", override=True)
...
class BackendAPIClient:
    ...
    def telegram_login(self, *, telegram_id: int, username: str, chat_id: int, identifier: str, password: str) -> dict[str, Any]:
    ...
    def update_interests(self, telegram_id: int, interests: list[str], threshold: int = 6) -> dict[str, Any]:
        return self._request(
            "PUT",
...
async def ensure_linked_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> dict[str, Any] | None:
    ...
    await send_long_message(
        ...
        "Нажмите <b>🔐 Войти</b>, если аккаунт уже есть, или <b>🆕 Регистрация</b>, чтобы создать новый.",
...
def news_text(payload: dict[str, Any]) -> str:
    ...
    if not items:
        return "По вашим интересам пока нет новостей."

    lines = ["<b>Персональная лента</b>"]
    for index, item in enumerate(items[:6], start=1):
        lines.append(
            f"\n{index}. <b>{escape(item.get('title'))}</b>\n"
            f"Источник: {escape(item.get('source') or 'не указан')}\n"
            f"Категория: {escape(item.get('category') or 'без категории')}\n"
            f"Важность: {escape(item.get('importance_score') or '—')}/10"
        )
        if item.get("summary"):
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user:
        await send_long_message(
            ...
            "Вы можете получать персональные новости, сводку и просто писать вопросы ИИ.",
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_long_message(
        ...
        "Если аккаунт привязан, вы можете просто писать сообщения, и бот ответит как ИИ-ассистент.\n"
        "Полезные примеры: <code>мои новости</code>, <code>мой профиль</code>, <code>сводка</code>, "
        "<code>обнови интересы: AI, Python</code>.",
        reply_markup=main_keyboard(bool(user)),
    )
...
async def handle_flow(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> bool:
    ...
    if flow == "login_password":
        try:
            ...
            await send_long_message(
                update,
                "Вход выполнен, Telegram привязан к аккаунту.\n\nТеперь можно сразу общаться с ИИ.",
                reply_markup=main_keyboard(True),
            )
        except BackendAPIError as exc:
            clear_flow(context)
            await send_long_message(update, f"Не удалось войти: {escape(exc)}", reply_markup=main_keyboard(False))
        return True

    if flow == "register_login":
        draft["login"] = text
        context.user_data["flow"] = "register_email"
        await send_long_message(update, "Шаг 2: введите email или отправьте <code>-</code>, если хотите пропустить.")
        return True

    if flow == "register_email":
        draft["email"] = "" if text == "-" else text
        context.user_data["flow"] = "register_password"
        await send_long_message(update, "Шаг 3: введите пароль не короче 6 символов.")
        return True

    if flow == "register_password":
        draft["password"] = text
        context.user_data["flow"] = "register_interests"
        await send_long_message(update, "Шаг 4: введите интересы через запятую.")
        return True

    if flow == "register_interests":
        draft["interests"] = parse_interests(text)
        context.user_data["flow"] = "register_threshold"
        await send_long_message(update, "Шаг 5: введите порог важности от 1 до 10. Если оставить пусто, будет 6.")
        return True
    ...
    if flow == "register_threshold":
        ...
        try:
            ...
            await send_long_message(
                update,
                "Регистрация завершена, Telegram сразу привязан к новому аккаунту.\n\nТеперь можно писать ИИ как обычному собеседнику.",
                reply_markup=main_keyboard(True),
            )
        except BackendAPIError as exc:
            clear_flow(context)
            await send_long_message(update, f"Не удалось зарегистрироваться: {escape(exc)}", reply_markup=main_keyboard(False))
        return True
    ...
    if flow == "update_interests":
        ...
        if not interests:
            clear_flow(context)
            await send_long_message(update, "Интересы не распознаны. Попробуйте снова из меню.", reply_markup=main_keyboard(True))
            return True
        try:
            payload = await call_backend(CLIENT.update_interests, telegram_id, interests, 6)
            ...
            clear_flow(context)
            await send_long_message(update, f"Не удалось обновить интересы: {escape(exc)}", reply_markup=main_keyboard(True))
        return True
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await send_long_message(update, f"Ошибка ИИ-ответа: {escape(exc)}", reply_markup=main_keyboard(True))
...
def main():
    if not CONFIG.token:
        raise RuntimeError("TELEGRAM_BOT_TOKEN is not set in .env or .env.bot")
